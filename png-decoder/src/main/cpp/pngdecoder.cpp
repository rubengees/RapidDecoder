    ...
    if (m_format == RGB565) {
        ...
        if (out) {
            ...
            for (int i = 0; i < col_length; ++i) {
                m_format.composer(out, 0xff, scanline_buffer[0], scanline_buffer[1],
                                  scanline_buffer[2]);
                scanline_buffer += 3;
            }
        }
        ...
        if (out) {
            ...
            for (int i = 0; i < col_length; ++i) {
                m_format.composer(out, scanline_buffer[3], scanline_buffer[0], scanline_buffer[1],
                                  scanline_buffer[2]);
                scanline_buffer += 4;
            }
        }
        ...
        if (m_col_offset == 0 && col_length >= m_width) {
            ...
            if (out) {
                memcpy(out, &scanline_buffer[m_col_offset * bytes_per_pixel],
                       col_length * bytes_per_pixel);
            }
        }
    }
...
void JNICALL Java_rapid_decoder_builtin_PngDecoder_init(JNIEnv *env, jclass clazz) {
    ...
    Bitmap_createBitmap1 = env->GetStaticMethodID(Bitmap, "createBitmap",
...
jobject JNICALL Java_rapid_decoder_builtin_PngDecoder_nativeDecode(JNIEnv *env, jclass clazz,
                                                                   jlong decoderPtr, jint left,
                                                                   jint top, jint right,
                                                                   jint bottom, jboolean filter,
                                                                   jobject config, jobject opts) {
    png_decoder *decoder = (png_decoder *) decoderPtr;
    ...
    jobject bitmap = env->CallStaticObjectMethod(Bitmap, Bitmap_createBitmap1, sampled_width,
                                                 sampled_height, config);
